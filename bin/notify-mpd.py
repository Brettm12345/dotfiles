...
try:
    ...
    print(
        "Can't import mpd. Please install the python bindings for MPD (python-mpd or mpd-python)"
    )
    sys.exit(1)
...
try:
    ...
    print(
        "Can't import pynotify so we'll use notify-send instead. We cannot update notifications using this method."
    )
...
try:
    ...
    import urllib2
    print(
        "This script requires a python module to daemonize notifications. Getting it now."
    )
    module = urllib2.urlopen(
        "https://raw.github.com/gist/1343075/faac02ebde4f22756857f3dbc2babd001d593303/daemon.py"
    )
...
try:
    ...
    import urllib2
    print(
        "This script requires a python module to download cover art. Getting it now."
    )
    module = urllib2.urlopen(
        "http://pycoverart.googlecode.com/svn-history/trunk/cover_art.py")
...
try:
    ...
    print(
        "Warning: Can't import Image. Please install the Python Imaging Library (probably python-imaging) if you want to use conky to display your cover-art. The cover art will then additionally be copied to a fixed location in your filesystem."
    )

__author__ = "Durand D'souza"
__email__ = "durand1 [at] gmail [dot] com"
__credits__ = [
    "Durand D'souza", ("Patrick Hirt",
                       "patrick [dot] hirt [at] gmail [dot] com"),
...
def cover_exists(artist, album, location):
    ...
    return glob.glob(u"{0}/{1}-{2}.*".format(location, sanitize(artist),
                                             sanitize(album)).replace(
                                                 "[", "\\[").replace(
                                                     "]", "\\]"))
...
def get_album_cover(artist, album):
    ...
    if cover_art_module:
        cover_path = cover_art.get_cover(artist, album,
                                         os.path.expanduser(COVERS_FOLDER))
    ...
    # Otherwise, use a simplified approach and don't download new art
        ...
        cover_path = cover_exists(artist, album,
                                  os.path.expanduser(COVERS_FOLDER))[0]
        if cover_path:
            return cover_path
    ...
    if pynotify_module:
        ...
        if update:
            ...
            notification = pynotify.Notification(
                summary=summary, message=message, icon=icon)
        notification.show()
    else:
        # Use notify-send instead
        os.popen('notify-send -i "{0}" "{1}" "{2}"'.format(
            icon.replace('"', '\\"'), summary.replace('"', '\\"'),
            message.replace('"', '\\"')))
...
def observe_mpd(client, log_file):
    ...
    while True:
        ...
        if current_status != last_status and last_status != "Initial":
            print(
                "{0}: Status change: {1}".format(
                    time.strftime("%a, %d %b %Y %H:%M:%S"), current_status),
                file=log_file)
        ...
        if (current_song != last_song and current_song != ""
            ) or (current_status != last_status
                  and current_status == "play") and last_song != "Initial":
            print(
                "{0}: Song change: {1} by {2} in {3}".format(
                    time.strftime("%a, %d %b %Y %H:%M:%S"), title, artist,
                    album),
                file=log_file)
            cover_art = get_album_cover(artist, album)
            send_notification(
                summary=artist, message=title, icon=cover_art, update=True)
            if COVER_ART_CONKY != "":
...
def run_notifier(self=None):
    ...
    while True:
        try:
            ...
            print(
                "{0}: Cannot connect to MPD".format(
                    time.strftime("%a, %d %b %Y %H:%M:%S")),
                file=log_file)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 2:
        ...
        if daemon_module:
            ...
            print(
                """Couldn't import the daemon module. Get the daemon module from http://is.gd/IXTtnT and save it next\
